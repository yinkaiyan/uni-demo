...
function processElement(ast, state, isRoot) {
    ...
    if (isRoot) {
        //百度 triggerEvent 不支持 bubbles，composed
        if (!state.options.isTest) {
            if (platformName !== 'mp-baidu' && platformName !== 'mp-alipay') {
                ast.attr['bind:' + INTERNAL_EVENT_LINK] = INTERNAL_EVENT_LINK
            }
        }
    }

    if (platformName === 'mp-alipay') { //form reset
        const resetCode = ast.attr['reset']
        if (resetCode && ast.type === 'form') {
            ast.attr['onReset'] = 'formReset'
            delete ast.attr['reset']
        }
    }
    ...
    if (state.options.platform.isComponent(ast.type)) {
        ...
        if (platformName === 'mp-alipay') { //传递函数处理 parent,children
            ast.attr['onTriggerLink'] = INTERNAL_EVENT_LINK
        }
        if (ast.attr.generic) {
