...
function getExtraDataPath (dataPath, methodName) {
  if (methodName === INTERNAL_SET_MODEL || methodName === INTERNAL_SET_SYNC) {
          ...
          if (!forExtra) {
            if (isForIndex(state.scoped, element)) {
              ...
              extraArrayElements.push(t.stringLiteral(
                getExtraDataPath(getCode(element), methodName)
              ))
            }
          } else {
...
function parseEvent (keyPath, valuePath, state, isComponent, isNativeOn = false, tagName, ret) {
  ...
  const isAlipayFormReset = state.options.platform.name === 'mp-alipay' &&
        type === 'reset' &&
        tagName === 'form'
  ...
  valuePath.forEach(funcPath => {
    if (funcPath.isIdentifier()) { // on:{click:handle}
      if (!isAlipayFormReset) {
        ...
        if (funcPath.node.name !== 'formReset') {
          state.errors.add(`支付宝小程序平台 form 组件 reset 事件仅支持 @reset="formReset" 方式绑定,其中方法名必须为 formReset`)
        }
      }
    } else if (isAlipayFormReset) {
      state.errors.add(`支付宝小程序平台 form 组件 reset 事件仅支持 @reset="formReset" 方式绑定,其中方法名必须为 formReset`)
