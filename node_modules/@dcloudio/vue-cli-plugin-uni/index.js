...
if (process.env.UNI_PLATFORM === 'app-plus') {
  if (platformOptions.usingNVue) {
    process.env.UNI_USING_NVUE = true
  }
}
...
// 输出编译器版本等信息
if (isInHBuilderX && process.env.UNI_PLATFORM === 'app-plus') {
  try {
    const modeText = platformOptions.usingComponents ? '自定义组件模式' : '非自定义组件模式'
    const pagesPkg = require('@dcloudio/webpack-uni-pages-loader/package.json')
    if (pagesPkg) {
      console.log('编译器版本：' + pagesPkg['uni-app']['compilerVersion'] + '，当前项目编译模式：' + modeText +
...
function getCopyWebpackPluginOptions () {
  ...
  if (process.env.UNI_PLATFORM === 'mp-weixin') { // TODO 微信自定义 tabBar，后续可能 app-plus 也要支持
    ['sitemap.json', 'ext.json', 'custom-tab-bar'].forEach(file => {
      appendCopyWebpackPluginOptions(
        options,
  ...
  if (process.env.UNI_PLATFORM === 'mp-weixin' || process.env.UNI_PLATFORM === 'app-plus') {
    // 微信原生小程序组件
    appendCopyWebpackPluginOptions(
      options,
