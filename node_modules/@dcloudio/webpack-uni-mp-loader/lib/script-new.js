...
module.exports = function (content) {
  ...
  const resourcePath = normalizeNodeModules(removeExt(normalizePath(path.relative(process.env.UNI_INPUT_DIR, this
    .resourcePath))))

  const type = resourcePath === 'App' ? 'App' : (process.UNI_ENTRY[resourcePath] ? 'Page' : 'Component')
  const {
    state: {
      components
    }
